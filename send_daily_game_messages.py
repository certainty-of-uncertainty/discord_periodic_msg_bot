...
class MyClient(commands.Bot):
  ...
  async def SendMessageToRpgBot(self):
    ...
    message = await channel.send("rpg daily", delete_after=60)
    await self.process_commands(message)
    time.sleep(5)
    message = await channel.send("rpg weekly", delete_after=60)
    await self.process_commands(message)
    time.sleep(5)
  ...
  async def SendMessageToMee6Bot(self):
    ...
    message = await channel.send("/daily", delete_after=60)
    await self.process_commands(message)
    time.sleep(5)
    message = await channel.send("/weekly", delete_after=60)
    await self.process_commands(message)
    time.sleep(5)
  ...
  async def SendMessageToFortniteBot(self):
    ...
    self.invoke()
    message = await channel.send("/claim-daily", delete_after=10)
    await self.process_commands(message)
    time.sleep(10)
